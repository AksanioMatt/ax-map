  ...
  methods: {
    ...
    async initMap() {
      ...
      try {
        ...
        this.zoomChanging(); //...M... call new method
        this.mouseUpBoundaryChanging(); //...M... call new method
      } catch (error) {
    ...
    mouseUpBoundaryChanging() {
      this.map.addListener("mouseup", () => {
        let mouseUpBoundsData = this.map.getBounds();
        mouseUpBoundsData = {"NorthEast":mouseUpBoundsData.getNorthEast(),"SouthWest":mouseUpBoundsData.getSouthWest()};
        // emit values from bounds changing
        this.$emit('trigger-event', {
